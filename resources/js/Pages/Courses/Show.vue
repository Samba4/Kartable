<template>
  <app-layout>
    ...
    <div class="max-w-7xl py-6 mx-auto sm:px-6 lg:px-8">
      ...
      <div class="py-6">
        <h1
          class="font-semibold text-xl text-gray-1200 leading-tight text-center"
        >
          Progression de {{ user.name }}
        </h1>
        </br>
      ...
      <div class="mt-6">
        <ul v-for="(episode, index) in course.episodes" v-bind:key="episode.id">
          <li class="mt-3 flex justify-between items-center">
            <div>
              Épisode n°0{{ index + 1 }} - {{ episode.title }}
              <button
